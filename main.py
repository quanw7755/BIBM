...
from dataset.medical_ad import MedicalADDataset
from dataset.mpdd import MPDDDataset
from dataset.mvtec_3d import MVTec3DDataset
...
def hard_background_loss(pred, gt, topk_ratio=0.05):
    pred = pred.float()
    gt = gt.float()
    ...
    bg_scores = pred_flat.masked_fill(gt_flat > 0.5, -1.0)

    k = max(1, int(bg_scores.size(1) * topk_ratio))
    topk_bg = torch.topk(bg_scores, k=k, dim=1).values
...
def train(args):
    ...
    test_dataset_kvasir = KvasirDataset(root=args.data_dir, train=False, category=None, transform=clip_transform,
                                        gt_target_transform=target_transform)
    test_dataset_brain_ad = MedicalADDataset(
        root=args.data_dir,
        train=False,
        category='Brain',
        transform=clip_transform,
        gt_target_transform=target_transform,
        dataset_name='brain_ad'
    )

    test_dataset_liver_ad = MedicalADDataset(
        root=args.data_dir,
        train=False,
        category='Liver',
        transform=clip_transform,
        gt_target_transform=target_transform,
        dataset_name='liver_ad'
    )

    test_dataset_resc = MedicalADDataset(
        root=args.data_dir,
        train=False,
        category='Retina_RESC',
        transform=clip_transform,
        gt_target_transform=target_transform,
        dataset_name='resc'
    )

    test_dataset_oct2017 = MedicalADDataset(
        root=args.data_dir,
        train=False,
        category='Retina_OCT2017',
        transform=clip_transform,
        gt_target_transform=target_transform,
        dataset_name='oct2017'
    )
    test_dataset_chest = MedicalADDataset(
        root=args.data_dir,
        train=False,
        category='Chest',
        transform=clip_transform,
    ...
    test_dataset_cvc300 = MedicalADDataset(
        root=args.data_dir,
        train=False,
        category='CVC-300',
        transform=clip_transform,
    ...
    all_test_dataset_dict = {
        ...
        "brain_ad": test_dataset_brain_ad,
        "liver_ad": test_dataset_liver_ad,
        "resc": test_dataset_resc,
        "oct2017": test_dataset_oct2017,
        "chest": test_dataset_chest,
        "cvc300": test_dataset_cvc300,

    }
    if len(args.test_dataset) < 1:
        test_dataset_dict = dict(all_test_dataset_dict)
    else:
        test_dataset_dict = {}
        for ds_name in args.test_dataset:
            test_dataset_dict[ds_name] = all_test_dataset_dict[ds_name]

    source_names = args.dataset  # 例如 ['mvtec', 'resc']

    train_datasets = [all_test_dataset_dict[name] for name in source_names]
    ...
    for ds_name in source_names:
        if ds_name in test_dataset_dict:
            del test_dataset_dict[ds_name]


    # if args.dataset == 'mvtec':
    #     train_dataset = test_dataset_mvtec
    # else:
    #     train_dataset = test_dataset_resc
    ...
    if args.weight is not None:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for items in tqdm(train_dataloader):
                ...
                loss = focal_loss(predict_labels, labels) + args.lambda1 * (
                            focal_loss(predict_masks, gts) + l1_loss(predict_masks,
                                                                     gts)) + args.lambda2 * patch_alignment_loss(
                    img_tokens, labels, gts)
                optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--dataset', type=str, default='mvtec', help='training dataset', choices=['mvtec', 'visa','resc'])
    parser.add_argument(
        '--dataset',
        nargs='+',
        type=str,
        default=['mvtec'],
        choices=['mvtec', 'visa', 'resc', 'btad', 'mpdd','mvtec_3d'],
        help='training source datasets'
    )
    # mvtec： clinic colon, resc, liver, oct17，chset
    # mvtec, visa: br35h brainmri kvasir
    # mvtec, mvtec-3d: isic
    ...
    parser.add_argument('--lambda2', type=float, default=0.1, help='lambda2 for loss')
    parser.add_argument('--tta', type=int, default=1, help='use test-time augmentation during evaluation')
    parser.add_argument('--use_freq', type=int, default=0)
    parser.add_argument('--use_hrami', type=int, default=1)
    parser.add_argument(
        '--freq_mode',
        type=str,
        default='both',
        choices=['both', 'low_only', 'high_only', 'wo_low', 'wo_high']
    )
    parser.add_argument(
        '--enhance_order',
        type=str,
        default='freq_hrami',
        choices=['freq_hrami', 'hrami_freq']
    )

    parser.add_argument('--freq_strength', type=float, default=1.0)
    parser.add_argument('--hrami_strength', type=float, default=1.0)

    parser.add_argument('--learnable_module_gates', type=int, default=0)

    parser.add_argument('--freq_cutoff', type=float, default=0.20)
    parser.add_argument('--hrami_grids', nargs='+', type=int, default=[2])
    parser.add_argument('--hrami_init_value', type=float, default=1e-3)
    parser.add_argument('--freq_init_gamma', type=float, default=1e-3)
    parser.add_argument('--smooth_map', type=int, default=0)
    parser.add_argument('--smooth_kernel', type=int, default=3)
    parser.add_argument('--cls_topk_ratio', type=float, default=0.03)
