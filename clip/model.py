...
import math
from clip.adaptor import Adaptor, FrequencyTokenEnhancer, HRAMI_AD
from clip.enhance_controller import TokenEnhanceController
...
class MultiScaleStructureAggregator(nn.Module):
    """
    Structure-aware multi-scale neighborhood aggregation.

    输入:
        x: [B, L, C], 其中第一个token为CLS，其余token为patch token
    输出:
        fused_tokens: list of [B, L, C]
            返回多个尺度增强后的token，保持与你当前代码兼容
    """
    def __init__(self, scales=(1, 3, 5), learnable_fusion=True):
        super().__init__()
        self.scales = scales
    ...
    def gaussian_kernel(self, size, sigma=4.0, device='cpu', dtype=torch.float32):
        ax = torch.arange(size, device=device, dtype=dtype) - size // 2
        xx, yy = torch.meshgrid(ax, ax, indexing='ij')
        kernel = torch.exp(-(xx ** 2 + yy ** 2) / (2 * sigma ** 2))
        kernel = kernel / kernel.sum()
        return kernel.reshape(1, size * size, 1)

    def aggregate_single_scale(self, x, patchsize):
        if patchsize == 1:
            return x
        ...
        h = w = int(math.sqrt(l))
        assert h * w == l, f"Patch token length {l} is not a square number."

        patch_tokens = patch_tokens.reshape(b, h, w, c).permute(0, 3, 1, 2)  # [B,C,H,W]
        ...
        unfolded = unfolded.permute(0, 2, 1).reshape(b * l, c, patchsize * patchsize).permute(0, 2, 1)
        # [B*L, k*k, C]

        kernel = self.gaussian_kernel(
            patchsize,
            sigma=float(patchsize),
            device=x.device,
            dtype=x.dtype
        )  # [1, k*k, 1]

        aggregated = torch.sum(unfolded * kernel, dim=1).reshape(b, l, c)
        aggregated = torch.cat([cls_token, aggregated], dim=1)
        return aggregated

    def forward(self, x):
        multi_scale_tokens = []
        for s in self.scales:
            multi_scale_tokens.append(self.aggregate_single_scale(x, s))

        # 返回多尺度结果，兼容你当前 detect_encode_image / store_memory 的列表流程
        return multi_scale_tokens

    def fused_forward(self, x):
        """
        如果你后续想做单输出融合版，可以用这个接口。
        """
        multi_scale_tokens = self.forward(x)

        if not self.learnable_fusion:
            return sum(multi_scale_tokens) / len(multi_scale_tokens)

        weights = torch.softmax(self.scale_logits, dim=0)
        fused = 0
        for w, tok in zip(weights, multi_scale_tokens):
            fused = fused + w * tok
        return fused
...
class CLIP(nn.Module):
    ...
    def insert(self, args, tokenizer, device):
        # self.normal_templates = f'without defect.'
        # self.anomaly_templates = f'with defect.'
        ...
        # ]
        #
        # self.num_normal = len(self.normal_templates)  # 1
        # self.num_anomaly = len(self.anomaly_templates)  # 1
        #
        ...
        self.state_prompt_embedding = nn.Parameter(
            torch.empty(1, args.prompt_len, self.token_embedding.weight.shape[-1], device=device)
        )
        nn.init.normal_(self.state_prompt_embedding, std=0.01)
        self.state_prompt_embedding.requires_grad_(True)

        token_dim = self.visual.proj.shape[0]

        self.enhance_controller = TokenEnhanceController(
            dim=token_dim,
            args=args,
            structure_aggregator_cls=MultiScaleStructureAggregator,
            freq_cls=FrequencyTokenEnhancer,
            hrami_cls=HRAMI_AD,
    ...
    def get_trainable_parameters(self):
        ...
        params += list(self.enhance_controller.parameters())
        params += list(self.adaptor.parameters())

        return params

    def _encode_ms_tokens(self, raw_tokens, project_for_text_branch=False):
        """
        raw_tokens:
            self.encode_image(...) 返回的多层 token list

        project_for_text_branch:
            True  -> detect_forward_seg，投影到 CLIP 文本对齐空间
            False -> memory bank，保留视觉 token 空间
        """
        all_tokens = self.enhance_controller(raw_tokens)

        if not project_for_text_branch:
            return all_tokens

        projected_tokens = []

        for tok in all_tokens:
            tok = self.visual.ln_post(self.adaptor(tok)) @ self.visual.proj
            projected_tokens.append(tok)
    ...
    def detect_encode_image(self, image, args):
        raw_tokens = self.encode_image(image, args.feature_layers)
        all_tokens = self._encode_ms_tokens(raw_tokens, project_for_text_branch=True)
        return all_tokens

    def store_memory(self, image, args):
        raw_tokens = self.encode_image(image, args.memory_layers)
        all_tokens = self._encode_ms_tokens(raw_tokens, project_for_text_branch=False)

        b, l, c = all_tokens[0].size()

        self.memorybank = [
            torch.nn.functional.normalize(tok[:, 1:], dim=-1).reshape(-1, c)
            for tok in all_tokens
        ]

    def aggregate_prompt_scores(self, scores):
        """
        scores: [B, L, K]
        K = num_normal + num_anomaly

        return:
            binary_scores: [B, L, 2]
            binary_scores[..., 0] = normal logit
            binary_scores[..., 1] = anomaly logit
        """
        normal_scores = scores[..., :self.num_normal]
        anomaly_scores = scores[..., self.num_normal:self.num_normal + self.num_anomaly]

        normal_logit = torch.logsumexp(normal_scores, dim=-1) - math.log(self.num_normal)
        anomaly_logit = torch.logsumexp(anomaly_scores, dim=-1) - math.log(self.num_anomaly)

        return torch.stack([normal_logit, anomaly_logit], dim=-1)
    ...
    def detect_forward_seg(self, image, args):
        ...
        # 多 prompt 聚合成 normal / anomaly 二分类
        binary_scores = self.aggregate_prompt_scores(scores)
        prob = torch.softmax(binary_scores, dim=-1)

        # CLS anomaly probability
        cls_prob = prob[:, 0, 1].view(-1)

        # patch anomaly probability
        patch_prob = prob[:, 1:, 1]

        # 用 anomaly map 的 top-k 生成 image-level anomaly score
        topk_ratio = getattr(args, "cls_topk_ratio", 0.03)
        k = max(1, int(patch_prob.size(1) * topk_ratio))
        map_prob = patch_prob.topk(k, dim=1).values.mean(dim=1)

        # CLS + map 融合
        # cls_weight 越小，越依赖 anomaly map
        cls_weight = getattr(args, "cls_weight", 0.2)
        cls_label = cls_weight * cls_prob + (1.0 - cls_weight) * map_prob

        b, l = patch_prob.size()
        h = w = int(math.sqrt(l))
        predict_map = patch_prob.reshape(b, 1, h, w)

        # 注意：这里建议只在 eval 阶段平滑，不要训练时平滑
        if (not self.training) and getattr(args, "smooth_map", 0):
            pad = args.smooth_kernel // 2
            predict_map = F.pad(predict_map, [pad, pad, pad, pad], mode="reflect")
            predict_map = F.avg_pool2d(
                predict_map,
    ...
    def detect_forward_memorybank(self, image, args):
        ...
        raw_tokens = self.encode_image(image, args.memory_layers)
        img_tokens = self._encode_ms_tokens(raw_tokens, project_for_text_branch=False)
