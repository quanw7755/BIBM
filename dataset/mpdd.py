"""
  @Author: 王权
  @FileName: mpdd.py
  @DateTime: 2026/1/14 12:20
  @SoftWare: PyCharm
"""
"""
  @Author: 王权
  @FileName: mpdd.py
  @DateTime: 2026/1/14 11:24
  @SoftWare: PyCharm
"""
"""dataset"""
import os
        ...
        for phase in ['train', 'test']:
            for category in self.categories:
                ...
                for defect_type in defect_types:
                    if defect_type == 'good':
                        img_paths = glob.glob(os.path.join(img_dir, phase, defect_type) + "/*.bmp") + glob.glob(
                            os.path.join(img_dir, phase, defect_type) + "/*.png")
                        ...
                        img_paths = glob.glob(os.path.join(img_dir, phase, defect_type) + "/*.bmp") + glob.glob(
                            os.path.join(img_dir, phase, defect_type) + "/*.png")
                        gt_paths = glob.glob(os.path.join(img_dir, 'ground_truth', defect_type) + "/*.bmp") + glob.glob(
                            os.path.join(img_dir, 'ground_truth', defect_type) + "/*.png")
                        img_paths.sort()
                        gt_paths.sort()
                        self.img_paths[phase][category].extend(img_paths)
